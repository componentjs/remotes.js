...
function malformedJSON(uri, res) {
  var err = new Error('bitbucket returned mailformed JSON at URL: ' + uri);
  err.res = res;
  err.text = res.text;
  err.remote = 'bitbucket';
  throw err;
...
function errorRateLimitExceeded(res) {
  var err = new Error('BitBucket rate limit exceeded. This is usually very unlikely to happen unless using the REST API to create forks or users - see https://answers.atlassian.com/questions/122958/bitbucket-rest-api-rate-limiting');
  err.res = res;
  err.remote = 'bitbucket';
  throw err;
}
