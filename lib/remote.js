...
function Remote(options) {
  ...
  // @api public
  this._version = Object.create(null)
...
Remote.prototype.versions = function* (repo) {
  ...
  if (repo in this._version)
    return this._version[repo] === 'resolving'
      ? (yield this.await(event))
      : this._version[repo]

  this._version[repo] = 'resolving'
  var references = yield* this._versions(repo)
  var versions = this._version[repo] = references
...
Remote.prototype.tree = function* (repo, ref) {
  ...
  if (slug in this._tree)
    return this._tree[slug] === 'resolving'
      ? (yield this.await(event))
      : this._tree[slug]

  this._tree[slug] = 'resolving'
  var tree = yield* this._trees(repo, ref)
  tree = tree || null
  this._tree[slug] = tree
